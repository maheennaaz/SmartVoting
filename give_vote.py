...
facedetect = cv2.CascadeClassifier(
    ...
    'haarcascade_frontalface_defaut.xml')
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        output = knn.oredict(resized_img)
        ts = time.time()
        date = datetime.fromtimestamp(ts).strftime("%d=%m-%Y")
        timestamp = datetime.fromtimestamp(ts).strftime("%H:%M-%S")
        exist = os.path.isfile("Votes" + ".csv")
        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 0, 255), 1)
        cv2.rectangle(frame, (x, y), (x+w, y+h), (50, 50, 25), 2)
        cv2.rectangle(frame, (x, y-40), (x+w, y), (50, 50, 25), -1)
        cv2.putText(
            frame, str(output[0]), (x, y-15), 
            cv2.FRONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 1)
